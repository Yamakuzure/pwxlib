...
const int32_t MAXINTCOUNT = 2000000;
...
int32_t testList(int32_t& testCount, int32_t& testSuccess, int32_t& testFail)
{
  ...
  if (EXIT_SUCCESS == result)
    {
      cout << adjRight(4,0) << ++testCount << " C) Remove elements 1, 3 and 5: ";
...
//      clock_t endTime   = 0;
//
//      cout << adjRight(4,0) << ++testCount << " E) Speed test (" << MAXINTCOUNT << " elements) : "; cout.flush();
//
