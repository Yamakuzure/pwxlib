...
int32_t main(int32_t argc, char** argv)
{
	...
	if ((argc < 3) || (argc > 4)) {
		cerr << "Usage:\n  " << basename(argv[0]) << " <destination directory> <number of hashes> [type]\n";
		cerr << " type is one of:\n";
		...
		cerr << "   int   : build cluster lists for [u]int16_t to [u]int64_t keys" << endl;
		return EXIT_FAILURE;
	}
	...
	try {
		...
		if (hash_type & 4) {
			...
			result = build_cluster_num<int16_t>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 4)) {
			...
			result = build_cluster_num<uint16_t>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 4)) {
			...
			result = build_cluster_num<int32_t>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 4)) {
			...
			result = build_cluster_num<uint32_t>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 4)) {
			...
			result = build_cluster_num<int64_t>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 4)) {
			...
			result = build_cluster_num<uint64_t>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 2)) {
			...
			result = build_cluster_num<float>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 2)) {
			...
			result = build_cluster_num<double>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 2)) {
			...
			result = build_cluster_num<long double>(destfile_chain, destfile_open, argMax);
		}
		...
		if ((EXIT_SUCCESS == result) && (hash_type & 1)) {
			...
			result = build_cluster_string(destfile_chain, destfile_open, argMax);
		}
